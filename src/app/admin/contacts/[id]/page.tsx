...
import { formatPhone, normalizePhone, formatDuration } from "@/lib/format";
...
export default function ContactProfilePage() {
  ...
  const [sourceReq, setSourceReq] = useState<SourceReq | null>(null);
  const [sourceReqBusy, setSourceReqBusy] = useState(false);
  const [sourceReqError, setSourceReqError] = useState("");
  ...
  const loadContact = useCallback(async () => {
    ...
    // Portal visit history (page_views recorded on /client since PageTracker
    // stopped excluding it) — only meaningful once they've registered.
    if (c.user_id) {
      fetch(`/api/admin/contact-activity?userId=${c.user_id}`)
    ...
    // Any pending "Lead Generated By" change awaiting two-party confirmation
    fetch(`/api/admin/lead-source-request?contactId=${id}`)
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data }) => {
      ...
      else { setMeEmail(data.user.email || ""); loadContact(); }
    });
  }, [router, loadContact]);
  ...
  async function saveEdit(e: React.FormEvent) {
    ...
    // "Lead Generated By" drives the commission split — it doesn't save
    // directly. If it changed, open a two-party confirmation request instead
    // and leave the stored value alone until both admins approve.
    const { sourced_by: nextSource, ...rest } = form;
    const sourceChanged = (nextSource || "") !== (contact.sourced_by || "");
    ...
    if (sourceChanged) {
      const res = await fetch("/api/admin/lead-source-request", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "create", contactId: contact.id, toValue: nextSource || null }),
      });
      const d = await res.json().catch(() => ({}));
      if (res.ok && d.applied) {
        setContact(c => c ? { ...c, sourced_by: nextSource || null } : c);
        setSourceReq(null);
      } else if (res.ok) {
        setSourceReq(d.request);
      } else {
        setSourceReqError(d.error || "Couldn't open the attribution-change request.");
      }
      // keep the form in sync with what's actually stored (still the old value)
      setForm(f => ({ ...f, sourced_by: (data?.sourced_by ?? contact.sourced_by) || "" }));
    }
  ...
  async function actOnSourceReq(action: "approve" | "reject" | "cancel") {
    if (!sourceReq) return;
    setSourceReqBusy(true);
    const res = await fetch("/api/admin/lead-source-request", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action, requestId: sourceReq.id }),
    });
    setSourceReqBusy(false);
    if (res.ok) { setSourceReq(null); loadContact(); }
  }
      ...
      <div className="max-w-[1500px] mx-auto px-4 md:px-8 pb-16">
        ...
        <div className="bg-[#111] border border-white/10 divide-y divide-white/5">
          ...
          <div className="p-5 grid grid-cols-2 gap-x-5 gap-y-4">
            ...
            {(contact.sourced_by || sourceReq) && (
              <div>
                <p className="text-[10px] tracking-[2px] uppercase text-[#444] mb-1">Lead Generated By</p>
                ...
                {sourceReq && (
                  <div className="mt-2 border border-[#fbbf24]/30 bg-[#fbbf24]/[0.06] rounded p-2.5 max-w-xs">
                    <p className="text-[11px] text-[#fbbf24] leading-snug">
                      Pending change: <b>{sourceReq.from_value || "—"}</b> → <b>{sourceReq.to_value || "—"}</b>
                    </p>
                    <p className="text-[10px] text-[#777] mt-0.5">Requested by {adminFirst(sourceReq.requested_by)}</p>
                    {sourceReq.requested_by === meEmail ? (
                      <div className="flex items-center gap-3 mt-2">
                        <span className="text-[10px] text-[#888]">Waiting for the other admin to confirm</span>
                        <button disabled={sourceReqBusy} onClick={() => actOnSourceReq("cancel")}
                          className="text-[10px] tracking-[1px] uppercase text-white/50 hover:text-white transition-colors disabled:opacity-40">Cancel</button>
                      </div>
                    ) : sourceReq.approved_by.includes(meEmail) ? (
                      <p className="text-[10px] text-[#4ade80] mt-2">You confirmed — applies once both agree</p>
                    ) : (
                      <div className="flex gap-2 mt-2">
                        <button disabled={sourceReqBusy} onClick={() => actOnSourceReq("approve")}
                          className="text-[10px] tracking-[1px] uppercase font-bold text-black bg-[#4ade80] hover:bg-[#34d399] px-2.5 py-1 transition-colors disabled:opacity-40">Confirm</button>
                        <button disabled={sourceReqBusy} onClick={() => actOnSourceReq("reject")}
                          className="text-[10px] tracking-[1px] uppercase text-red-400 border border-red-400/30 hover:bg-red-400/10 px-2.5 py-1 transition-colors disabled:opacity-40">Reject</button>
                      </div>
                    )}
                  </div>
                )}
        ...
        <div className="space-y-4">
          ...
          {historyTab === "activity" && (() => {
            type AnyEvent =
              ...
              | { kind: "visit"; ts: string; data: PortalVisit };

            function describeVisit(path: string): string {
              if (path === "/client") return "Portal — Home";
              if (path.startsWith("/client/gallery/")) {
                const shootId = path.split("/client/gallery/")[1]?.split(/[?#]/)[0];
                const shoot = shoots.find(s => s.id === shootId);
                return shoot ? `Viewed Gallery — ${shoot.address}` : "Viewed a Shoot Gallery";
              }
              if (path.includes("tab=invoices")) return "Portal — Invoices";
              if (path.startsWith("/client")) return "Portal";
              return path;
            }
            ...
            const events: AnyEvent[] = [
              ...
              ...visits.map(v => ({ kind: "visit" as const, ts: v.created_at, data: v })),
                        ...
                        <div className="flex gap-4 pb-4 pl-2">
                          ...
                          <div className="shrink-0 w-9 flex flex-col items-center pt-1">
                            <div className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] z-10 relative ${
                              ...
                              event.kind === "visit" ? "bg-[#94a3b8]/20 border border-[#94a3b8]/40 text-[#94a3b8]" :
                                                       "bg-[#34d399]/20 border border-[#34d399]/40 text-[#34d399]"
                            }`}>
                              {event.kind === "shoot" ? "📷" : event.kind === "call" ? "📞" : event.kind === "email" ? "✉" : event.kind === "inquiry" ? "🌐" : event.kind === "followup" ? "✓" : event.kind === "reply" ? "↩" : event.kind === "visit" ? "👁" : "💬"}
                            </div>
                          </div>
                          ...
                          <div className="flex-1 bg-[#111] border border-white/8 rounded-sm p-3 min-w-0">
                            <div className="flex items-start justify-between gap-2 mb-1.5">
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {event.kind === "visit" && (
                                  <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold tracking-wide uppercase text-[#94a3b8] bg-[#94a3b8]/10 border border-[#94a3b8]/20">
                                    Portal Visit
                                  </span>
                                )}
                              </div>
                            ...
                            {event.kind === "visit" && (() => {
                              const v = event.data as PortalVisit;
                              return (
                                <div>
                                  <p className="text-sm font-medium">{describeVisit(v.path)}</p>
                                  <p className="text-xs text-[#555] mt-0.5">{formatDuration(v.duration_seconds)} spent on page</p>
                                </div>
                              );
                            })()}
                            {event.kind === "followup" && (() => {
            ...
            <form onSubmit={saveEdit} className="space-y-3">
              ...
              <div>
                <select value={form.sourced_by} onChange={e => setForm(f => ({ ...f, sourced_by: e.target.value }))}
                  disabled={!!sourceReq}
                  className="w-full bg-[#181818] border border-white/10 text-white text-sm px-4 py-2.5 outline-none focus:border-white/30 disabled:opacity-50">
                  <option value="">Lead Generated By…</option>
                  <option value="Ryan">Ryan</option>
                  <option value="Leif">Leif</option>
                </select>
                <p className="text-[10px] text-[#666] mt-1">
                  {sourceReq
                    ? `A change to ${sourceReq.to_value || "—"} is already awaiting confirmation.`
                    : "Changing this opens a request that both Ryan and Leif must confirm."}
                </p>
                {sourceReqError && <p className="text-[10px] text-red-400 mt-1">{sourceReqError}</p>}
              </div>

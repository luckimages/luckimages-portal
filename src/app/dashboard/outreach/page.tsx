...
type EmailLogRow = { contact_id: string | null; subject: string | null; sent_at: string | null; category: string | null; sent_by?: string | null };
type LinkClickRow = { id: string; contact_id: string | null; service: string; clicked_at: string };
...
export default function OutreachPage() {
  ...
  const [senderFilter, setSenderFilter] = useState<"all" | "ryan" | "leif">("all");
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [{ data: logs }, { data: clicks }, { data: textLogs }, pageViewsRes] = await Promise.all([
        supabase.from("email_log").select("contact_id, subject, sent_at, category, sent_by").not("contact_id", "is", null).order("sent_at", { ascending: false }),
        ...
        // Only cold_calls rows generated by the "Copy Text Follow-up Link"
        // button (marked via notes) — not every manual "Sent Text" tag ever
        // logged the old way, which never had a tracked link behind it.
        supabase.from("cold_calls").select("contact_id, outcome, called_at, called_by").not("contact_id", "is", null).ilike("outcome", "%sent_text%").ilike("notes", `%${COLD_CALL_TEXT_LINK_NOTE}%`).order("called_at", { ascending: false }),
      ...
      const textEntries: EmailLogRow[] = (textLogs || []).map(t => ({
        contact_id: t.contact_id, subject: null, sent_at: t.called_at, category: "Cold Call Texts", sent_by: t.called_by,
      }));
  ...
  // Two eras, split by the moment the dwell pipeline became the sole source of
  // truth:
  //   • Clicks BEFORE the cutoff — the dwell link (page_views.link_click_id)
  //     didn't reliably exist yet, so every historical click is trusted and
  //     shown exactly as recorded. Do not retroactively hide these.
  //   • Clicks ON/AFTER the cutoff — a real click always runs the landing-page
  //     JS and produces a dwell row. No dwell data => bot prefetch (iMessage/
  //     WhatsApp preview fetch, crawler) => not shown.
  // Dwell data, when present, also drives the engaged/duration colouring.
  const DWELL_REQUIRED_FROM = new Date("2026-09-09T23:30:00Z").getTime();
  const isRealClick = (c: { id: string; clicked_at?: string }) => {
    if (dwellByClickId[c.id] !== undefined) return true;
    if (c.clicked_at && new Date(c.clicked_at).getTime() < DWELL_REQUIRED_FROM) return true;
    return false;
  };
  ...
  const engagement = (() => {
    const emailLogView = senderFilter === "all"
      ? emailLog
      : emailLog.filter(l => (l.sent_by || "").toLowerCase() === senderFilter);
    ...
    for (const l of emailLogView) if (l.contact_id) bucket(l.contact_id).emails.push(l);
      ...
      {mode === "engagement" ? (
        <div className="flex-1 min-h-0 flex flex-col overflow-visible md:overflow-hidden">
          ...
          <div className="px-6 md:px-8 pt-4 shrink-0 flex items-center gap-2 flex-wrap border-b border-white/10 pb-3">
            ...
            {/* Filter everything below by who did the outreach */}
            <div className="ml-auto flex items-center gap-1 border border-white/10 p-0.5">
              {(["all", "ryan", "leif"] as const).map(s => (
                <button key={s} onClick={() => setSenderFilter(s)}
                  className={`text-[10px] tracking-[1px] uppercase px-2.5 py-1 transition-colors ${
                    senderFilter === s ? "bg-white text-black font-bold" : "text-[#666] hover:text-white"
                  }`}>
                  {s === "all" ? "Both" : s}
                </button>
              ))}
                              ...
                              <div key={i} className="border-l-2 border-white/10 pl-4 pt-2">
                                <div className="flex items-baseline justify-between gap-3 flex-wrap">
                                  <span className="text-sm font-semibold text-[#4ade80]">
                                    ...
                                    {email.sent_by && <span className="ml-2 text-[10px] tracking-[1px] uppercase text-[#666] font-normal">· {email.sent_by}</span>}
                                  </span>

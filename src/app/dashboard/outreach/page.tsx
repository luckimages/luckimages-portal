  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [{ data: logs }, { data: clicks }, { data: textLogs }] = await Promise.all([
        ...
        supabase.from("cold_calls").select("contact_id, outcome, called_at").not("contact_id", "is", null).ilike("outcome", "%sent_text%").eq("notes", COLD_CALL_TEXT_LINK_NOTE).order("called_at", { ascending: false }),
      ]);

    ...
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd}>
      <div className="flex gap-3 overflow-x-auto pb-2">
        {lists.map(list => {
          ...
          return (
            <div key={list.id} className={`bg-[#111] border flex flex-col flex-shrink-0 w-64 transition-colors ${isOver ? "border-white/30 bg-white/[0.03]" : "border-white/10"}`} style={{ minHeight: 280 }}>
              {/* List header */}
        ...
        {lists.length < 5 && (
          <button onClick={handleCreateList}
            className="flex-shrink-0 w-52 border border-dashed border-white/10 hover:border-white/20 text-[#333] hover:text-[#666] transition-colors text-xs flex items-center justify-center gap-1.5 min-h-[80px]">
            + New List
          </button>
        )}
      </div>

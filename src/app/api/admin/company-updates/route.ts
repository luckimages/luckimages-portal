...
export async function GET(req: Request) {
  ...
  const mappedPosts = (posts || []).map((p: { id: string; message: string; created_at: string; created_by: string; link?: string; category?: string }) => ({
    id: p.id,
    type: p.created_by === "system" ? "tool" : "post",
    category: p.category || (p.created_by === "system" ? "nocturne" : "nocturne"),
    message: p.message,

...
export async function notifyDelivery(shootId: string): Promise<void> {
  ...
  if (CLIENT_EMAILS_ENABLED && resendKey && clientEmail) {
    // Same hero-image + dark-overlay background and dual-signature footer as
    // the Mass Invite / portal-invite email (src/lib/portalInviteEmail.ts) —
    // kept in sync by eye since the two templates serve different flows and
    // don't share a component.
    ...
    <tr><td align="center" style="padding:48px 24px;">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;">
        <tr><td style="border:1px solid rgba(255,255,255,0.15);padding:40px;background:rgba(12,12,12,0.55);">
          ...
          <p style="margin:0 0 8px;font-size:14px;line-height:1.6;color:#ccc;">${shoot.address}</p>
          <p style="margin:0 0 28px;font-size:14px;line-height:1.6;color:#ccc;">Your media is uploaded and ready to view and download in your portal.${amountStr && !invoice?.paid ? ` Pay the ${amountStr} invoice to unlock full-resolution downloads.` : ""}</p>
